...
#
# Usage:
# Place data.txt in the working directory (or setwd() before running).
...
# The script expects a folder named "data" next to data-prep.R (i.e. .../OpenSesame-online/data). 
# Do not move or rename that folder.
# If data.txt is missing, the script prompts a warning explaining how to
# rename the JATOS export to data.txt and where to put it.
script_path <- NULL
...
if (!dir.exists(data_dir)) {
  stop("Required 'data' folder not found next to data-prep.R:\n  ",
       normalizePath(if (!is.null(script_path)) script_dir else getwd()),
       "\nPlease ensure the repository was decompressed and the 'data' folder is present next to data-prep.R. Do not move the data folder.")
}

setwd(data_dir)

# Friendly hint if the JATOS export has a different name
if (!file.exists("data.txt")) {
  warning("File 'data.txt' not found in the repository data/ folder.\n",
          "If your data export from JATOS has a different name, please rename it to 'data.txt' and place it inside the 'data' folder that comes with the decompressed GitHub repository.\n",
          "The script requires the file to be called 'data.txt' and stored in the repo's data/ folder which is located next to this data-prep.R script (do not change the data folder location).")
}
...
# -------- Read input -------------------------------------------------------
input_file <- "data.txt"
if (!file.exists(input_file)) stop("Input file not found: 'data.txt'")

json <- tryCatch(fromJSON(input_file, flatten = TRUE),
                 error = function(e) stop("Failed to read JSON: ", e$message, call. = FALSE))

if (is.null(json$data)) stop("JSON does not contain a top-level 'data' element.")
